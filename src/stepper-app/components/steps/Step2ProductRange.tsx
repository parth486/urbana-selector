...
export const Step2ProductRange: React.FC<Step2Props> = ({ 
  data, 
  productGroup, 
  selection, 
  onSelect 
  ...
  return (
    <div>
      <p className="text-default-600 mb-6">
        Select a product range from the {productGroup} category:
      </p>
                ...
                <div className="flex flex-col sm:flex-row">
                  ...
                  <div className="p-4 flex-1">
                    ...
                    <p className="text-default-500 text-sm mt-2">
                      {getRangeDescription(range, productGroup)}
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {getFeatureTags(range, productGroup).map((tag) => (
                        <span 
                          key={tag} 
                          className="text-xs px-2 py-1 bg-default-100 text-default-700 rounded-medium"
                        >
                          {tag}
                        </span>
                      ))}
...
function getRangeDescription(range: string, productGroup: string): string {
  const descriptions: Record<string, string> = {
    ...
    "Decorative": "Aesthetic lighting features that enhance landscapes",
    "Security": "High visibility lighting for safety and security",
    "Pathway": "Directional lighting for walkways and pedestrian areas"
  };
